...
for isub = 1:length(who_idx)
    ...
    if ismember('interp_chans',EP.S.Properties.VariableNames)
        ...
        choice = questdlg(sprintf([...
                            'Your "SubjectsTable" spreadsheet tells me\n',...
                            ...
                            'Yes, interpolate!', ...
                            'No, continue without interpolation', ...
                            'No, continue without interpolation');
        % Handle response
        if strcmp(choice,'No, continue without interpolation')
            interp_chans = [];
        end
    else
        interp_chans = [];
    ...
    %  ---------------------------------------------------------------------
    [EEG, com] = pop_selectiveinterp(EEG,badChnXtrl);
    EEG = eegh(com,EEG);
